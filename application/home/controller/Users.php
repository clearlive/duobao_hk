...
//        $data = input('get.');
//        $uid = $this->uid;
//        $identification = '6HU7G42HN55F94UV';
//        $token = '3284326W8PZ3QZUD7YXTZJBTTHY6U48G';
//        $orderuid = $uid;
...
//            $type = 1;
//        }
//        $notify_url = 'http://'.$_SERVER['HTTP_HOST'].'/home/payments/notify';
//        $return_url = 'http://'.$_SERVER['HTTP_HOST'].'/home/users/index';
//        $orderid = $uid.'_'.date('YmdHis');
//        $key=md5($identification.$notify_url.$orderid.$orderuid.$price.$return_url.$token.$type);//MD5密钥
//        $array = [
...
//        ];
//
//        $this->postman('https://pay.mmbbo.cn/',json_encode($array));
//       return redirect('https://pay.mmbbo.cn/');
        ...
        $return_url = 'http://'.$_SERVER['HTTP_HOST'].'/home/users/index';
        $notify_url = 'http://'.$_SERVER['HTTP_HOST'].'/home/payments/mq_notify';
        $orderuid = $uid;
        //验证key,不可以更改参数顺序。
        ...
        if ($type == 1){
            ...
            $types = '支付宝支付';
                ...
                db('recharge')->insert(['bptype'=>3,'bpbalance'=>sprintf("%.2f",0),'createTime'=>date('Y-m-d H:i:s'),'bpprice'=>sprintf("%.2f",$_POST['price']),'uid'=>$uid,'remarks'=>'会员充值','isverified'=>0,'balance_sn'=>$orderid,'pay_type'=>$types]);
